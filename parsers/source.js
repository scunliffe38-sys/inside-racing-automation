...
export const EXPECTED = [
  ...
  { file: 'Add Twilight in Race Program.xlsx', section: 'Race programs', cadence: 'monthly', required: false },
...
export async function pickFolder() {
  // read and write: the console bakes edition.json back into the same folder,
  // which is what the assembled edition and the print copy then read
  const handle = await window.showDirectoryPicker({ id: 'inside-racing-inputs', mode: 'readwrite' });
  dir = handle;
  mode = 'folder';
  map = null;
  root = [];
...
export async function reconnect() {
  if (!dir) return state();
  let perm = 'denied';
  try { perm = await dir.requestPermission({ mode: 'readwrite' }); } catch (e) { /* fall through */ }
  if (perm === 'granted') { mode = 'folder'; needsPermission = false; install(); }
  return state();
}

/** Write a file into the chosen folder. Used for the bake. */
export async function write(name, text) {
  if (mode !== 'folder' || !dir) throw new Error('no folder attached');
  let base = dir;
  for (const seg of root) { try { base = await base.getDirectoryHandle(seg); } catch (e) { break; } }
  const h = await base.getFileHandle(name, { create: true });
